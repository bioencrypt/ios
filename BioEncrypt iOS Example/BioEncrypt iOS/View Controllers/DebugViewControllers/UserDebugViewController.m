...
- (void)viewDidLayoutSubviews {
    ...
    // Scroll to top
    [self.userDebugOutput setContentOffset:CGPointZero animated:NO];
}
...
- (void)setText {
    ...
    for(BETrustFactorOutputObject *trustFactorOutputObject in self.computationResults.userTrustFactorsAttributingToScore){
    ...
    for(BETrustFactorOutputObject *trustFactorOutputObject in self.computationResults.userTrustFactorWhitelist){
        ...
        userTrustFactorsToWhitelist = [userTrustFactorsToWhitelist stringByAppendingFormat:@"--Name: %@\n\nWeight Applied:%ld\nWeight Percent:%.02g\nUse Partial:%@\nTotal Possible:%@\n\nCurrent Assertion:\n%@Matching Assertions:\n%@",trustFactorOutputObject.trustFactor.name, (long)trustFactorOutputObject.appliedWeight,trustFactorOutputObject.percentAppliedWeight,trustFactorOutputObject.trustFactor.partialWeight,trustFactorOutputObject.trustFactor.weight,currentAssertions,storedAssertions];
    ...
    for(BETrustFactorOutputObject *trustFactorOutputObject in self.computationResults.userTrustFactorsNotLearned){
    ...
    for(BETrustFactorOutputObject *trustFactorOutputObject in self.computationResults.userTrustFactorsWithErrors){
    ...
    for(BETrustFactorOutputObject *trustFactorOutputObject in self.computationResults.transparentAuthenticationTrustFactorOutputObjects){
        ...
        trustFactorsForTransparentAuth = [trustFactorsForTransparentAuth stringByAppendingFormat:@"--Name: %@\n\nCurrent Assertion:\n%@",trustFactorOutputObject.trustFactor.name, currentAssertions];
        
    }
    complete = [complete stringByAppendingString:trustFactorsForTransparentAuth];
